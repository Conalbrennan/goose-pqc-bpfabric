...
// Store learned source MAC addresses and the switch ports on which they were observed
...
// Handle traffic entering and returning from the user-space encryption path
...
// Redirect matching GOOSE traffic from the physical network towards the encryption TAP path
//Traffic from physical ports and only GOOSE traffic 
if (pkt->metadata.in_port == 0 && pkt->eth.h_proto == 47240) 
{       
    return PORT + 3;  // Forwards traffic to virtual port 0 to GEDSF
    //PORT + 1  
    //bpf_mirror(0, pkt, pkt->metadata.length); // Virtual port      
    //return DROP; 
}

// Perform standard Layer 2 MAC learning and forwarding for traffic not redirected above
