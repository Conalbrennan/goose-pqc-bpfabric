...
# Implements the server side of the authenticated post-quantum key exchange.
# ML-KEM establishes the shared secret, ML-DSA authenticates the exchange,
# and HKDF derives the session material used by the GOOSE decryption endpoint.
...
# Define the cryptographic algorithms, protocol parameters and GOOSE group configuration
...
# Define the locations of the server identity, trusted client key and active session record
...
# Provide common encoding, file handling and length-prefixed socket messaging functions
...
# Sign outgoing protocol messages and verify incoming ML-DSA authenticated messages
def sign_message(
...
# Derive the operational AES key, confirmation key and nonce prefix from the ML-KEM shared secret
...
# Generate the persistent ML-DSA server identity used to authenticate later handshakes
...
# Run one authenticated KEM exchange and create the server-side decryption session
...
# Generate the ephemeral ML-KEM key pair and construct the signed server offer
...
with oqs.KeyEncapsulation(KEM_ALGORITHM) as kem:
    ...
    # Accept the client connection, exchange authenticated KEM messages and validate group membership
    with socket.socket(
        socket.AF_INET,
        socket.SOCK_STREAM,
    ) as listener:
        ...
        with connection:
            ...
            # Decapsulate the ML-KEM ciphertext and bind the derived keys to the authenticated transcript
            ...
            # Verify that the client derived the same session material before activating the session
            ...
            # Sign the final confirmation and store the active session for the decryption forwarder
            final_core = {
                "type": "KEM_COMPLETE",
                "status": "ACTIVE",
                "version": PROTOCOL_VERSION,
...
# Parse command-line options and either initialise the identity or run the KEM server
def main() -> int:
