...
# Handles the per-packet encryption stage of the GOOSE protection pipeline.
# Original GOOSE frames are read from tap_enc_in, encrypted using the active
# authenticated session, and written as protected frames to tap_enc_out.
...
# Identify GOOSE frames and construct authenticated session metadata for AES-GCM
...
# Save per-packet AES-GCM timing measurements for later performance analysis
def save_results():
    os.makedirs(
...
# Process each GOOSE frame, construct a unique nonce and encrypt only the GOOSE payload
try:
while True:
    ...
    # Everything after the Ethernet header is the original GOOSE payload.
    original_payload = (
        frame_data[ETHERNET_HEADER_SIZE:]
    )
    ...
    # Construct the outgoing frame directly from raw bytes. No Scapy packet object is created or serialised.
    encrypted_frame = (
        ethernet_header
        + counter_bytes
        + encrypted_payload
