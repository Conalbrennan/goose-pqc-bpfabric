...
# Implements the client side of the authenticated post-quantum key exchange.
# ML-KEM establishes the shared secret, ML-DSA authenticates the exchange,
# and HKDF derives the session material used by the GOOSE encryption endpoint.
...
# Reuse the shared protocol constants and helper functions from kem_key_server.py for simplicity and to ensure both endpoints use identical message handling, cryptographic parameters and session-derivation logic.
...
# Define the locations of the client identity, trusted server key and active session record
...
# Generate the persistent ML-DSA client identity used to authenticate later handshakes
...
# Run the authenticated KEM exchange and create the client-side encryption session
...
# Warm up HKDF before collecting the cryptographic timing measurements
# HKDF WARM-UP
derive_session_material(
    os.urandom(32),
    os.urandom(32),
...
# Measure the complete online handshake from connection establishment to session activation
# TIMING: overall handshake begins immediately before attempting the TCP connection.
...
with socket.create_connection(
    ...
    # Receive and authenticate the server offer before accepting its ML-KEM public key
    ...
    # Encapsulate to the authenticated ML-KEM public key and derive matching session material
    ...
    # Prove possession of the derived session material and sign the client KEM response
    ...
    # Authenticate the server's final response and confirm that both sides derived the same session
    ...
    # Store the active encryption session for later use by encrypt_forwarder.py
    session_record = {
        "status": "ACTIVE",
        "role": "encryption",
        "session_id": offer["session_id"],
    ...
    # Report successful protocol completion and the measured handshake components
...
# Parse command-line options and either initialise the identity or run the KEM client
def main() -> int:
